...
#
#  Usage: ./Launcherv3_resumption_batch.sh [tls|quic] [none|simple|stable|unstable] [loss-percent] [delay-ms]
###############################################################################
...
# N par phase, total = N*2
BATCH_SIZE=500
RUNS_PER_TEST=$((BATCH_SIZE * 2))
...
RESULTS_HOST_DIR="/home/tolokoum/Documents/TLS-QUIC/resumption_study/results"
...
echo "=============================================================================="
echo "  BATCH SEPARÉS : Phase 1 (N full) → Phase 2 (N resumed)"
echo "=============================================================================="
echo "  Protocol:        $PROTOCOL"
echo "  Batch size:      $BATCH_SIZE per phase"
echo "  Total runs:      $RUNS_PER_TEST ($BATCH_SIZE full + $BATCH_SIZE resumed)"
...
for SIG_ALG in "${!PAIRS[@]}"; do
    ...
    # Lancer le client batch
    echo "[CLIENT] Starting batch test..."
    ...
    docker run --cap-add=NET_ADMIN \
        ...
        -e SIG_ALG=$SIG_ALG \
        -e USE_TLS=$USE_TLS \
        -e NUM_RUNS=$BATCH_SIZE \
        -e MUTUAL=false \
        -e CLIENT_ID=1 \
        -e RESULTS_DIR=/results \
        $IMAGE ./perftestClientResumptionBatch.sh
...
echo ""
echo "=============================================================================="
echo "  BATCH SEPARÉ TESTS COMPLETED"
