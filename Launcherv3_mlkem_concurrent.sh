...
#
#  Ce script mesure les performances des handshakes post-quantiques sous charge
#  concurrente (10, 50, 100, 500 clients) avec collecte complète des métriques:
...
#    - Context switches
#
#  Usage: ./Launcherv3_concurrent.sh [tls|quic] [num_clients] [none|simple|stable|unstable] [loss-percent] [delay-ms] [--classic]
#
#  Options:
#    --classic : Utilise les signatures/KEMs classiques (Ed25519 + x25519)
#                Par défaut, utilise les versions post-quantiques (ML-DSA65 + ML-KEM768)
#
#  Exemples:
#    # PQ complet: ML-DSA65 + ML-KEM768
#    ./Launcherv3_concurrent.sh tls 10 none 0 0
#
#    # Baseline classique: Ed25519 + x25519
#    ./Launcherv3_concurrent.sh tls 10 none 0 0 --classic
#
#    # QUIC avec ML-DSA65 + HQC-192 (si disponible)
#    ./Launcherv3_concurrent.sh quic 10 none 0 0 --hqc
#
#    # Test avec 500 clients et modèle GE stable
#    ./Launcherv3_concurrent.sh tls 500 stable 0 0
###############################################################################
...
RUNS_PER_CLIENT=500
MONITOR_DURATION=$((RUNS_PER_CLIENT * 10 / 1000 + 60))  # Estimation durée + marge

# =============================================================================
#  CONFIGURATION DES ALGORITHMES
# =============================================================================
# Selon les options, on sélectionne les bonnes primitives
if $USE_CLASSIC; then
    # Baseline classique (Ed25519 + x25519)
    SUPPORTED_SIG_ALGS=("ed25519")
    KEMS=("x25519")
    TEST_TYPE="CLASSIC_BASELINE"
elif $USE_HQC; then
    ...
    # Configuration PQ par défaut (ML-DSA65 + ML-KEM768)
    SUPPORTED_SIG_ALGS=("mldsa65")
    KEMS=("mlkem768")
    TEST_TYPE="PQ_MLKEM"
fi
...
# =============================================================================
USAGE="Usage: $0 [tls|quic] [num_clients] [none|simple|stable|unstable] [loss-percent] [delay-ms] [--classic] [--hqc]"
...
RESULTS_BASE_DIR="/home/tolokoum/Documents/TLS-QUIC/mldsa-mlkem-study1/result_concurrent"
...
# =============================================================================
echo "=============================================================================="
echo "  CONCURRENT LOAD TEST - $TEST_TYPE"
echo "=============================================================================="
...
start_monitoring() {
    ...
    echo "[MONITOR] Starting resource monitoring..."
...
stop_monitoring() {
    ...
    if [ -f "${RESULTS_RUN_DIR}/docker_stats.log" ]; then
        ...
        # CPU moyen des conteneurs
        grep -E "cliente|servidor" "${RESULTS_RUN_DIR}/docker_stats.log" | \
            grep -oE '[0-9]+\.[0-9]+%' | \
            awk '{sum+=$1; count++} END {if(count>0) print "Average container CPU: " sum/count "%"}' \
            >> "${RESULTS_RUN_DIR}/resource_summary.txt"
        
        # Mémoire moyenne des conteneurs
        grep -E "cliente|servidor" "${RESULTS_RUN_DIR}/docker_stats.log" | \
            grep -oE '[0-9]+\.[0-9]+[KMG]iB' | \
            head -20 >> "${RESULTS_RUN_DIR}/memory_samples.txt" 2>/dev/null || true
    fi
}
...
for SIG_ALG in "${SUPPORTED_SIG_ALGS[@]}"; do
    ...
    # Générer les certificats (sauf pour les signatures classiques qui utilisent les certificats existants)
    ...
    for KEM in "${KEMS[@]}"; do
        ...
        case "$NETWORK_PROFILE" in
            ...
            stable|unstable)
                ...
                # Note: pumba doit être installé sur l'hôte
