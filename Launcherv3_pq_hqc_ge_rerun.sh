            ...
            case "$NETWORK_PROFILE" in
              ...
              stable|unstable)
                ...
                /usr/local/bin/pumba netem --duration 1h --interface $NETIF \
                  ...
                  sleep 1
                if kill -0 "${PUMBA_PIDS_SERVER[-1]}" 2>/dev/null; then
                  echo "   ✔ Pumba PID ${PUMBA_PIDS_SERVER[-1]} confirmed running (SIG=$SIG_ALG KEM=$KEM PROTO=$PROTOCOL)" | tee -a ge_rerun_pumba_verification.log
                else
                  echo "   ✘ FATAL: Pumba failed to start (SIG=$SIG_ALG KEM=$KEM PROTO=$PROTOCOL)" | tee -a ge_rerun_pumba_verification.log
                  exit 1
                fi
                ;;
            esac
