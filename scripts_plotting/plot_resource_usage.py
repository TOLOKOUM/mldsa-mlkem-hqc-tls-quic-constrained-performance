...
SOURCE DES DONNÉES ET DÉDUPLICATION :
    Certains fichiers contiennent plusieurs mesures pour une même
    combinaison (re-runs partiels). Règle appliquée : la ligne au
    timestamp le plus récent est conservée par combinaison
    (protocol, auth_mode, sig_alg, kem, role) -- rapportée explicitement
    en console, jamais silencieuse (cf. resolve_duplicate_runs, plot_style.py).

    Pas de distribution statistique ici (une valeur -- déjà moyennée sur
    n_runs pour le CPU, un pic pour la mémoire -- par combinaison), donc
    pas de violon ni de barre d'erreur possible, comme pour traffic size.
"""
...
EXPECTED_COLUMNS = ["timestamp", "protocol", "auth_mode", "sig_alg", "kem",
...
def load_resource_csv(csv_path: Path):
    ...
    obscurément ou de mal aligner des colonnes)."""
    ...
    bad_rows = [i for i, r in enumerate(rows[1:], start=2) if len(r) not in (n_expected, n_expected)]
...
def draw_grouped_bars(ax, sub_df: pd.DataFrame, value_col: str, unit_scale: float):
    ...
    hachure=rôle. Retourne le max de valeur tracée (pour ylim)."""
    ...
    for role, offset in zip(ROLE_ORDER, offsets):
        ...
        for kem in kems:
            if kem in role_df.index:
                values.append(role_df.loc[kem, value_col] * unit_scale)
            else:
                values.append(0.0)
...
def build_metric_figure(root: Path, protocol, auth_mode, scenario_dir, scenario_slug,
    ...
    if metric_key == "cpu":
        value_col, unit_scale, ylabel, log_scale = "cpu_usec_per_handshake", 1 / 1000.0, \
            "CPU time per handshake (ms), log scale", True
    else:
        value_col, unit_scale, ylabel, log_scale = "mem_peak_bytes", 1 / (1024 * 1024), \
    ...
    for ax, sig_alg in zip(axes, sig_algs):
        ...
        top = draw_grouped_bars(ax, sig_rows, value_col, unit_scale)
